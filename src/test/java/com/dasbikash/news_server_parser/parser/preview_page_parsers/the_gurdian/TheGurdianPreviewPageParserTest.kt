...
internal class TheGurdianPreviewPageParserTest {
    ...
    fun readFirstPageArticles(){
        ...
        newsPapers.filter {
            ...
            it?.let {
                PreviewPageParserFactory.getPreviewLoaderByNewsPaper(newspaper = newsPapers.get(0))?.loadPreview(PreviewPageParseRequest(it,1))
                        ?.forEach {
                            println("Article  ${it}")
                        }
            }
