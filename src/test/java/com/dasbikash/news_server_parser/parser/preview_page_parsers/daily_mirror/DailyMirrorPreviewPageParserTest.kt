...
internal class DailyMirrorPreviewPageParserTest {
    ...
    fun readFirstPageArticles(){
        ...
        newsPapers.filter {
            it.id == NEWS_PAPER_ID.DAILY_MIRROR.id
        }.map {
            println("Np: ${it?.name}")
            ...
            it?.let {
                DailyMirrorPreviewPageParser().loadPreview(PreviewPageParseRequest(it,1))
                        ?.forEach {
                            println("Article  ${it}")
                        }
            }
