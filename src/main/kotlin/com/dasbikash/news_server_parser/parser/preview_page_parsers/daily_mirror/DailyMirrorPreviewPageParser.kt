...
class DailyMirrorPreviewPageParser : PreviewPageParser() {

    private val mSiteBaseAddress = "https://www.mirror.co.uk"

//    internal var mArticleLinkBlocks: Elements
    var mArticleLinkBlock: Element?=null
    ...
    override fun getArticleLink(previewBlock: Element): String? {
        val mArticleLinkBlocks = previewBlock.select(DailyMirrorPreviewPageParserInfo.ARTICLE_LINK_ELEMENT_SELECTOR)
        if (mArticleLinkBlocks.size != 1) return null
        mArticleLinkBlock = mArticleLinkBlocks[0]
    ...
    override fun getArticlePreviewImageLink(previewBlock: Element): String {
        return previewBlock.select(DailyMirrorPreviewPageParserInfo.ARTICLE_PREVIEW_IMAGE_LINK_ELEMENT_SELECTOR)[0].attr(DailyMirrorPreviewPageParserInfo.ARTICLE_PREVIEW_IMAGE_LINK_TEXT_SELECTOR_TAG)
    }
