...
class DawnPreviewPageParser : PreviewPageParser() {
    ...
    override fun getArticleLink(previewBlock: Element): String {
        return previewBlock.select(DawnPreviewPageParserInfo.ARTICLE_LINK_ELEMENT_SELECTOR)[0].attr(DawnPreviewPageParserInfo.ARTICLE_LINK_TEXT_SELECTOR_TAG)
    }
    ...
    override fun getArticleTitle(previewBlock: Element): String {
        return previewBlock.select(DawnPreviewPageParserInfo.ARTICLE_TITLE_ELEMENT_SELECTOR)[0].text()
    }

    override fun getArticlePublicationDateString(previewBlock: Element): String {
        return mPageLink.substring(mPageLink.length - mCurrentPage.linkVariablePartFormat!!.length)
    }
